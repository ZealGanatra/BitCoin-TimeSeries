...
def computeDelta(wt, X, Xi):
    Xi_spliced = Xi.iloc[:, :-1]

    Xi_spliced_mean = Xi_spliced.mean(axis=1).to_frame()
    Xi_spliced_std = Xi_spliced.std(ddof=0, axis=1).to_frame()

    M = len(X)-1

    X_spliced = X.to_frame().iloc[:-1, :]
    X_spliced_mean = X_spliced.mean()
    X_spliced_std = X_spliced.std(ddof=0)

    similar_A = pd.DataFrame(Xi_spliced.values - Xi_spliced_mean.values)
    similar_B = pd.DataFrame(X_spliced.values - X_spliced_mean.values)  # .transpose()
    similar_mat = similar_A.dot(similar_B)

    similar_mat_sum = similar_mat.sum(axis=1).to_frame()

    denominator = M * X_spliced_std.values[0] * Xi_spliced_std
    sim = np.exp(wt * (similar_mat_sum.div(denominator)))
    sim_y = sim.transpose().dot(((Xi.iloc[:, -1].to_frame())))
    ans = sim_y.sum().values[0] / sim.sum().values[0]
...
computeDelta(weight, test_90.iloc[1], train1_90)
